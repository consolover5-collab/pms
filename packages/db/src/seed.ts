...
// Canonical UUIDs. Tests and seed-refs.ts pin these; keep in sync if regenerating.
// Anything not listed here is free to be auto-generated by the database.
const CANONICAL = {
  property: "ff1d9135-dfb9-4baa-be46-0e739cd26dad",
  roomType: {
    STD: "e8f25fcd-bdaf-43bb-b39f-4d9ad6d83c84",
    STD_TWN: "689f969c-d956-4138-8b63-0490c11e1f90",
    SUP: "11111111-aaaa-4bbb-8ccc-000000000001",
    PRM: "22222222-aaaa-4bbb-8ccc-000000000002",
    JRS: "0589e980-b720-4ba1-a29a-ad695865797f",
    STE: "33333333-aaaa-4bbb-8ccc-000000000003",
  },
  ratePlan: {
    RACK: "f47aaf7f-572e-4c24-9f66-506bd24839be",
    PROMO: "28d39f1c-87bd-4824-b3e8-788053b6ff37",
    CORP: "8f23168b-4ac0-47cf-b671-2137bc857f3c",
    BAR: "9f92f036-e7c1-41a4-98a2-1987f8cac2ce",
    LONG: "55555555-aaaa-4bbb-8ccc-000000000005",
  },
  room: {
    "204": "66666666-aaaa-4bbb-8ccc-000000000204",
    "206": "e3413f3d-a212-495e-b606-5410929e6d37",
    "704": "d601fe24-4bfb-4e86-b936-2dee00489080",
    oos: "6762e1df-44b0-48cf-915e-97ac366cc297",
  },
  guest: {
    alexanderFedorov: "7f170421-0f77-4e04-a973-1b1f95ecc79b",
  },
  txCode: {
    BREAKFAST: "83b646b7-ec41-4879-965c-990e3d17b649",
    ROOM: "c4d15cc0-d75e-4d3c-ae5c-a38fcc4f6a2e",
    MINIBAR: "25462d33-7748-4725-bbe3-3484e53557fe",
    PARKING: "d87a8b30-df0a-4eb7-94a6-ac7462360b97",
  },
  pkg: {
    BKFST: "df6fab35-0f22-445c-b202-2c162660b105",
    PARK: "02db4d8f-675a-4ce0-954f-6c441e6bbc4e",
  },
  hkTask: {
    room204Stayover: "00eb7a7d-1786-424f-9e06-11373aca35ee",
    room704Stayover: "907ed90b-48a5-4bd3-bd96-d212a40e6d9f",
  },
} as const;
...
async function seed() {
  ...
  const types = await db
    .insert(roomTypes)
    .values([
      { id: CANONICAL.roomType.STD, propertyId: property.id, name: "Standard Double", code: "STD", maxOccupancy: 2, baseRate: "4500.00", description: "Standard room with double bed", sortOrder: 1 },
      { id: CANONICAL.roomType.STD_TWN, propertyId: property.id, name: "Standard Twin", code: "STD_TWN", maxOccupancy: 2, baseRate: "4500.00", description: "Standard room with two single beds", sortOrder: 2 },
      { id: CANONICAL.roomType.SUP, propertyId: property.id, name: "Superior", code: "SUP", maxOccupancy: 2, baseRate: "5500.00", description: "Superior room with city view", sortOrder: 3 },
      { id: CANONICAL.roomType.PRM, propertyId: property.id, name: "Premium", code: "PRM", maxOccupancy: 2, baseRate: "6500.00", description: "Premium room with upgraded amenities", sortOrder: 4 },
      { id: CANONICAL.roomType.JRS, propertyId: property.id, name: "Junior Suite", code: "JRS", maxOccupancy: 3, baseRate: "8500.00", description: "Junior suite with separate living area", sortOrder: 5 },
      { id: CANONICAL.roomType.STE, propertyId: property.id, name: "Suite", code: "STE", maxOccupancy: 4, baseRate: "12000.00", description: "Full suite with living room and bedroom", sortOrder: 6 },
    ])
    .returning();
  ...
  // Rooms whose UUIDs tests pin. Other rooms get auto-generated IDs.
  // Room 205 is the canonical OOS anchor (seeded out_of_service below).
  const PINNED_ROOM_IDS: Record<string, string> = {
    "204": CANONICAL.room["204"],
    "205": CANONICAL.room.oos,
    "206": CANONICAL.room["206"],
    "704": CANONICAL.room["704"],
  };
  const insertedRooms = await db
    .insert(rooms)
    .values(
      roomData.map((r) => ({
        ...(PINNED_ROOM_IDS[r.roomNumber] ? { id: PINNED_ROOM_IDS[r.roomNumber] } : {}),
  ...
  // Room 206 is reserved as the canonical "dirty vacant STD" anchor for test 06.
  // Room 205 is reserved as the canonical OOS anchor. Neither should be assigned
  // to bookings by the random pool, otherwise the deterministic state is lost.
  const RESERVED_ROOM_NUMBERS = new Set(["205", "206"]);
  ...
  for (const r of insertedRooms) {
    if (RESERVED_ROOM_NUMBERS.has(r.roomNumber)) continue;
    const type = types.find((t) => t.id === r.roomTypeId)!;
    roomsByType[type.code].push({ id: r.id, number: r.roomNumber });
  }

  // ============ Guest profiles (130 individuals) ============
  // First entry is the canonical sample guest Alexander Fedorov (tests pin this UUID).
  const sampleGuestPhone = String(pickInt(0, 9999999)).padStart(7, "0");
  const sampleGuest = {
    id: CANONICAL.guest.alexanderFedorov,
    propertyId: property.id,
    type: "individual" as const,
    firstName: "Alexander",
    lastName: "Fedorov",
    name: "Alexander Fedorov",
    email: "alexander.fedorov@example.com",
    phone: `+7 ${pickInt(900, 999)} ${sampleGuestPhone}`,
  ...
  const rateData = await db
    .insert(ratePlans)
    .values([
      { id: CANONICAL.ratePlan.RACK, propertyId: property.id, code: "RACK", name: "Rack Rate", description: "Standard published rate", baseRate: "5000.00", isDefault: true, isActive: true },
      { id: CANONICAL.ratePlan.PROMO, propertyId: property.id, code: "PROMO", name: "Promotional Rate", description: "Discounted promotional rate", baseRate: "4000.00", isActive: true },
      { id: CANONICAL.ratePlan.CORP, propertyId: property.id, code: "CORP", name: "Corporate Rate", description: "Corporate discount rate", baseRate: "4500.00", isActive: true },
      { id: CANONICAL.ratePlan.BAR, propertyId: property.id, code: "BAR", name: "Best Available Rate", description: "Best available rate for the day", baseRate: "4700.00", isActive: true },
      { id: CANONICAL.ratePlan.LONG, propertyId: property.id, code: "LONG", name: "Long Stay (7+ nights)", description: "Discount for stays of 7+ nights", baseRate: "3800.00", isActive: true },
    ])
    .returning();
  ...
  // Canonical OOS anchor — room 205 is always out_of_service with the
  // fixed OOS UUID. Test 06-checkin-checkout pins this pair.
  await db
    .update(rooms)
    .set({
    ...
    .where(eq(rooms.id, CANONICAL.room.oos));

  // Canonical dirty+vacant anchor — room 206 for test 06 DIRTY_ROOM_ID +
  // test 08 probe (needs at least one dirty+vacant room in the grid).
  await db
    .update(rooms)
    .set({
    ...
    .where(eq(rooms.id, CANONICAL.room["206"]));

  // Two additional OOO/OOS rooms from the random pool (excluding the anchor).
  const oooCandidates = shuffle(
    insertedRooms.filter((r) => r.id !== CANONICAL.room.oos),
  ).slice(0, 2);
  ...
  await db.insert(transactionCodes).values([
    { id: CANONICAL.txCode.ROOM, propertyId: property.id, code: "ROOM", description: "Room Charge", groupCode: "ROOM", transactionType: "charge", isManualPostAllowed: false, adjustmentCodeId: adjMap["ADJ_ROOM"], sortOrder: 10 },
    ...
    { id: CANONICAL.txCode.MINIBAR, propertyId: property.id, code: "MINIBAR", description: "Minibar Charge", groupCode: "FB", transactionType: "charge", isManualPostAllowed: true, adjustmentCodeId: adjMap["ADJ_FB"], sortOrder: 32 },
    { id: CANONICAL.txCode.BREAKFAST, propertyId: property.id, code: "BREAKFAST", description: "Breakfast", groupCode: "FB", transactionType: "charge", isManualPostAllowed: true, adjustmentCodeId: adjMap["ADJ_FB"], sortOrder: 33 },
    { id: CANONICAL.txCode.PARKING, propertyId: property.id, code: "PARKING", description: "Parking", groupCode: "MISC", transactionType: "charge", isManualPostAllowed: true, sortOrder: 34 },
  ]);
  ...
  const pkgData = await db
    .insert(packages)
    .values([
      { id: CANONICAL.pkg.BKFST, propertyId: property.id, code: "BKFST", name: "Breakfast", transactionCodeId: txCodeMap["BREAKFAST"], calculationRule: "per_person_per_night", amount: "800.00", postingRhythm: "every_night", isActive: true },
      { id: CANONICAL.pkg.PARK, propertyId: property.id, code: "PARK", name: "Parking", transactionCodeId: txCodeMap["PARKING"], calculationRule: "per_night", amount: "500.00", postingRhythm: "every_night", isActive: true },
    ])
    .returning();
  ...
  const vacantPool = insertedRooms.filter(
    (r) =>
      !usedRooms.has(r.id) &&
      // Reserve rooms 204 and 704 for the canonical stayover_clean anchors below.
      r.roomNumber !== "204" &&
      r.roomNumber !== "704",
  );
  ...
  // Canonical stayover_clean tasks pinned by test 09-housekeeping. Fixed UUIDs
  // so re-seeds don't invalidate TASK_ASSIGN_ID/TASK_COMPLETE_ID references.
  const rooms204And704 = insertedRooms.filter(
    (r) => r.roomNumber === "204" || r.roomNumber === "704",
  );
  const room204 = rooms204And704.find((r) => r.roomNumber === "204");
  const room704 = rooms204And704.find((r) => r.roomNumber === "704");
  if (room204) {
    hkRows.push({
      id: CANONICAL.hkTask.room204Stayover,
      propertyId: property.id,
      roomId: room204.id,
      businessDateId: bizDate.id,
      taskType: "stayover_clean",
  ...
  if (room704) {
    hkRows.push({
      id: CANONICAL.hkTask.room704Stayover,
      propertyId: property.id,
      roomId: room704.id,
      businessDateId: bizDate.id,
      taskType: "stayover_clean",
